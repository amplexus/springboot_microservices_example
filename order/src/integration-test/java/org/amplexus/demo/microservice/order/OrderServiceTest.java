...
public class OrderServiceTest {
    ...
    @Test
    public void testGetWithNoOrdersShouldReturnEmptySet() {
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
        ResponseEntity<String> response = new TestRestTemplate().exchange("http://localhost:" + this.port + "/orders", HttpMethod.GET, new HttpEntity<Void>(headers), String.class);
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertThat(response.getBody(), equalTo("[]"));
    }

    @Test
    public void testGetWithNoOrdersShouldReturnEmptySetV2() throws Exception {
